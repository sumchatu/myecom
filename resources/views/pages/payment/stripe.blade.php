                ...
                <div class="col-lg-5" style="border: 1px solid grey; padding: 20px; border-radius: 25px;">
					<div class="contact_form_container">
						...
						<form action="{{route('stripe.payment')}}" method="post" id="payment-form">
							...
							@if(Session::get('coupon'))
								<input type="hidden" name="total" value="{{(Cart::Subtotal() + $setting->shipping_charge + $setting->vat)- Session::get('coupon')['discount']}}">
							@else
								<input type="hidden" name="total" value="{{Cart::Subtotal() + $setting->shipping_charge + $setting->vat}}">
							@endif
...
<script>
	...
	var stripe = Stripe('pk_test_51H1CFLBWYiAmbMxSbSoPZh1OCiNkPIIsmS8LXijHZsA1V07NFKlmYh4namHwvDwMhHoVfVHOuguRFFBWRNL7eWn1006fajH7la');
